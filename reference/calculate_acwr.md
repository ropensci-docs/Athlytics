...
ratio (ACWR). The ACWR helps identify periods when recent load is
elevated relative to chronic load.
...
- **Reference Band**: ACWR between 0.8-1.3 (commonly used load-balance
  band)
...
- missing_load:
  ...
  be computed (e.g. an HRSS call on an activity with no HR samples, or a
  TSS call without FTP). Options:

  - `"zero"` (default): coalesce to 0, matching the historical
    behaviour. This keeps backwards compatibility but conflates
    missing-data training days with genuine rest days.
...
monitoring training load balance in athletes (Gabbett, 2016; Hulin et
al., 2016).
...
entirely (Impellizzeri et al., 2021). Users should interpret ACWR zones
as descriptive heuristics rather than validated injury predictors.
...
#> Warning: Earliest activity (2026-07-02) is 334 day(s) after the required history-buffer start (2025-08-02). ACWR for roughly the first 333 day(s) of the analysis window will be NA or weakly anchored because the baseline has no prior data.
print(head(result))
#> # A tibble: 6 × 5
#>   date         atl   ctl  acwr acwr_smooth
#>   <date>     <dbl> <dbl> <dbl>       <dbl>
#> 1 2026-07-02    NA    NA    NA          NA
#> 2 2026-07-03    NA    NA    NA          NA
#> 3 2026-07-04    NA    NA    NA          NA
#> 4 2026-07-05    NA    NA    NA          NA
#> 5 2026-07-06    NA    NA    NA          NA
#> 6 2026-07-07    NA    NA    NA          NA
