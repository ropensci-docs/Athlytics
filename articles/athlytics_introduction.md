...
[README](https://github.com/ropensci/Athlytics#installation). The quick
version:
...
Start](https://github.com/ropensci/Athlytics#quick-start).
...
Keep the ZIP path — Athlytics reads ZIP files directly. Stream-based
functions such as
...
reuse this same path passed as `export_dir`.
...
- **1.3-1.5** - Elevated ACWR ratio; recent load increasing faster than
  chronic load
- **\> 1.5** - High ACWR ratio; descriptive load-spike band, not a
  validated injury-prediction rule
...
2.  **Sharp spikes above 1.5** = large recent/chronic-load imbalance;
    interpret with training context
3.  **Extended periods \< 0.8** = recent load below the chronic-load
    baseline
4.  **Stable values in 0.8-1.3** = values within a commonly used
    reference band
...
framework entirely (Impellizzeri et al., 2021). **Use ACWR zones as
descriptive workload heuristics rather than validated injury
predictors.**
...
1.  **Time-based rolling window**: Window size is derived from the
    stream’s median sampling interval, targeting a 300-second centered
    window while respecting minimum and maximum row caps.
...
3.  **Contiguous steady-state blocks**: Data points where CV \<
    `steady_cv_threshold` (default 8%) are grouped into continuous
    steady-state blocks rather than pooled across scattered intervals.
4.  **Minimum duration**: EF and decoupling require a contiguous
    steady-state block spanning at least `min_steady_minutes` (default
    20 minutes for EF, 40 minutes for decoupling).
5.  **Aggregation**: EF and decoupling are computed from the accepted
    steady-state block, using median-based aggregation to reduce
...
rate gradually rises during prolonged efforts even if speed/power
remains constant. Low decoupling indicates good aerobic endurance.
...
>   training-load state space, showing whether a high ACWR comes from
>   ramping up on a low chronic-load base or from a natural fluctuation
>   around a high chronic-load base.
>
...
- **Red zone** = High ACWR ratio; descriptive load-spike zone, not a
...
- **GitHub Issues**: [Report
  bugs](https://github.com/ropensci/Athlytics/issues)
...
- **[Advanced Features
  Tutorial](https://docs.ropensci.org/Athlytics/articles/advanced_features.md)**
  — EWMA-based ACWR with confidence intervals, quality control, and
  cohort analysis
- **[Function
  Reference](https://docs.ropensci.org/Athlytics/reference/index.html)**
...
1.  **Cohort Studies**: See
    [calculate_cohort_reference()](https://docs.ropensci.org/Athlytics/reference/calculate_cohort_reference.html)
    for multi-athlete percentile comparisons
2.  **Data Quality**: Use
    [flag_quality()](https://docs.ropensci.org/Athlytics/reference/flag_quality.html)
...
#> [1] ggplot2_4.0.3   Athlytics_1.0.6
#> 
...
#> [21] lubridate_1.9.5    bslib_0.12.0       pillar_1.11.1      RColorBrewer_1.1-3
#> [25] rlang_1.3.0        cachem_1.1.0       xfun_0.60          fs_2.1.0          
#> [29] sass_0.4.10        S7_0.2.2           otel_0.2.0         timechange_0.4.0  
#> [33] cli_3.6.6          mgcv_1.9-4         pkgdown_2.2.1      withr_3.0.3       
#> [37] magrittr_2.0.5     digest_0.6.39      grid_4.6.0         nlme_3.1-169      
